...
public sealed class ContextPackSelectionService : IContextPackSelectionService
{
    public ContextPackSelectionResult Select(
        ContextPackRequest request_,
        string generatedAtLocal_)
    {
        List<string> warnings = [];
        ContextPack pack = this.BuildPack(request_, generatedAtLocal_, warnings);
        return new ContextPackSelectionResult(pack, warnings);
    }
    ...
    public ContextPackInventoryCompatibility GetInventoryCompatibility(string repoRoot_)
    {
        ...
        return new ContextPackInventoryCompatibility(symbolCompatible, endpointCompatible);
    }

    private ContextPack BuildPack(
        ContextPackRequest request_,
        string generatedAtLocal_,
        List<string> warnings_)
    {
        if (request_.Task == "changed-files")
        {
            ...
            return this.BuildChangedFilesPack(request_, generatedAtLocal_, changedSymbols, warnings_);
        }
    ...
    private ContextPack BuildChangedFilesPack(
        ContextPackRequest request_,
        string generatedAtLocal_,
        JsonArray symbols_,
        List<string> warnings_)
    ...
    {
        ContextPackInventoryCompatibility compatibility = this.GetInventoryCompatibility(request_.RepoRoot);
    ...
    private IReadOnlyList<string> GetRiskAreas(
        ContextPackRequest request_,
        JsonObject? buildRoot_,
        JsonObject? secretRoot_,
        JsonObject? manifestRoot_,
        IReadOnlyList<ContextPackItem> symbols_,
        IReadOnlyList<ContextPackItem> packages_)
    {
        List<string> risks = [];
    ...
    private string GetSummary(
        ContextPackRequest request_,
        IReadOnlyList<ContextPackItem> symbols_,
        IReadOnlyList<ContextPackItem> endpoints_,
        IReadOnlyList<ContextPackItem> packages_,
        IReadOnlyList<string> risks_)
    {
