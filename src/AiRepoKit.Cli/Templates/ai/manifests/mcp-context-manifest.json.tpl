{
  "schemaVersion": "0.2",
  "repoName": {{RepoNameJson}},
  "repoRoot": "<target-repo>",
  "mainSolution": {{MainSolutionJson}},
  "generatedAtLocal": {{GeneratedAtLocalJson}},
  "targetFramework": {{TargetFrameworkJson}},
  "mcpServerName": {{McpServerNameJson}},
  "mcpProjectRelativePath": {{McpProjectRelativePathJson}},
  "allowedContextFiles": [
    ".ai/README.md",
    ".ai/ai-operating-rules.md",
    ".ai/automation-risks.md",
    ".ai/build-profile.md",
    ".ai/codex-usage-guide.md",
    ".ai/project-map.md",
    ".ai/sdk-profile.md",
    ".ai/test-profile.md",
    ".ai/tool-roadmap.md",
    ".ai/context-budget.json",
    ".ai/generated/inventories/project-inventory.json",
    ".ai/generated/inventories/project-references.json",
    ".ai/generated/inventories/package-inventory.json",
    ".ai/generated/inventories/sdk-inventory.json",
    ".ai/generated/inventories/symbol-inventory.json",
    ".ai/generated/inventories/symbol-inventory.md",
    ".ai/generated/inventories/endpoint-inventory.json",
    ".ai/generated/inventories/endpoint-inventory.md",
    ".ai/generated/context-packs/changed-files.json",
    ".ai/generated/context-packs/changed-files.md",
    ".ai/generated/graphs/project-graph.json",
    ".ai/generated/graphs/project-graph.md",
    ".ai/generated/graphs/symbol-graph.json",
    ".ai/generated/graphs/symbol-graph.md",
    ".ai/generated/graphs/risk-graph.json",
    ".ai/generated/graphs/risk-graph.md",
    ".ai/generated/reports/impact-report.json",
    ".ai/generated/reports/impact-report.md",
    ".ai/generated/reports/mcp-budget-report.json",
    ".ai/generated/reports/mcp-budget-report.md",
    ".ai/generated/reports/org-scan.json",
    ".ai/generated/reports/org-scan.md",
    ".ai/generated/reports/org-report.json",
    ".ai/generated/reports/org-report.md",
    ".ai/generated/reports/org-efficiency.json",
    ".ai/generated/reports/org-efficiency.md",
    ".ai/generated/reports/sdk-alignment-report.json",
    ".ai/generated/reports/secret-scan-report.json",
    ".ai/generated/reports/build-diagnostics-report.json",
    ".ai/generated/reports/latest-build-summary.json",
    ".ai/generated/summaries/generated-context-summary.md",
    ".ai/manifests/mcp-context-manifest.md",
    ".ai/playbooks/change-ui.md",
    ".ai/playbooks/diagnose-build.md",
    ".ai/playbooks/update-package.md",
    ".ai/playbooks/inspect-security.md"
  ],
  "restrictedPaths": [
    ".git",
    ".vs",
    ".vscode",
    ".idea",
    "bin",
    "obj",
    "artifacts",
    ".tmp",
    ".dotnet-home",
    ".codex/config.toml",
    "Logs",
    "AutoGenerated",
    "oracle-data",
    "wwwroot",
    ".build-*",
    "appsettings*.json",
    "docker-compose*.yml",
    "docker-compose*.yaml",
    "*.key",
    "*.pem",
    "*.pfx",
    "*.jks",
    "*.keystore",
    "migrations"
  ],
  "budgets": {
    "compactBytes": 8192,
    "fullBytes": 49152,
    "combinedBytes": 65536,
    "fileReadBytes": 1048576,
    "searchDefaultLimit": 5,
    "searchHardLimit": 25,
    "arrayDefaultLimit": 25,
    "arrayHardLimit": 100,
    "previewChars": 240
  },
  "recommendedContextBudgets": {
    "symbol-inventory": {
      "briefBytes": 8192
    },
    "endpoint-inventory": {
      "briefBytes": 8192
    }
  }
}
